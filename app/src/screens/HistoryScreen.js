    ...
    <div className={`card fade-up`} style={{ animationDelay: `${index * 0.04}s`, opacity: 0, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          {entry.srwneResult && (
            <span style={{ fontSize: 11, fontFamily: 'var(--font-sans)', color: entry.srwneResult === 'autonomous' ? 'var(--sage)' : 'var(--clay)', background: entry.srwneResult === 'autonomous' ? 'rgba(122,174,142,0.1)' : 'rgba(201,125,80,0.1)', padding: '3px 8px', borderRadius: 20 }}>
